...
public class TwoManLowNetScorer extends AbstractTeamGameScorer {
    ...
    public RoundGameResult scoreRound(RoundScoringData data) {
        ...
        for (TeamScoringData team : data.getTeams()) {
            requireExactPlayerCount(team, 2, "Two-Man Low Net");
            ...
            for (int holeNumber = 1; holeNumber <= 18; holeNumber++) {
                ...
                if (!hasEnoughScores(grosses, 1) || !hasEnoughScores(nets, 1)) {
                    continue;
                }

                int holeGross = sumLowest(grosses, 1);
                int holeNet = sumLowest(nets, 1);
