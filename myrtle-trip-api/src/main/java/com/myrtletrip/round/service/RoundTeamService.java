...
public class RoundTeamService {
    ...
    public List<RoundTeamResponse> saveTeams(Long roundId, SaveRoundTeamsRequest request) {
        ...
        if (format == null) {
            throw new IllegalStateException("Round format is not set");
        }

        if (!format.requiresTeams()) {
            throw new IllegalStateException("This round format does not use teams");
        }
    ...
    public List<RoundTeamResponse> getTeams(Long roundId) {
        ...
        if (!round.getFormat().requiresTeams()) {
            return List.of();
        }
    ...
    private void validateRequest(SaveRoundTeamsRequest request, RoundFormat format, Round round) {
        ...
        for (RoundTeamRequest team : request.getTeams()) {
            if (team.getTeamNumber() == null) {
                throw new IllegalArgumentException("Each team must have a teamNumber");
            }

            if (!teamNumbers.add(team.getTeamNumber())) {
                throw new IllegalArgumentException("Duplicate teamNumber: " + team.getTeamNumber());
            }
            ...
            for (RoundTeamPlayerRequest player : team.getPlayers()) {
                ...
                if (!playerIds.add(player.getPlayerId())) {
                    throw new IllegalArgumentException("Player assigned more than once: " + player.getPlayerId());
                }

                if (player.getPlayerOrder() == null) {
                    throw new IllegalArgumentException("Each team player must have a playerOrder");
                }
